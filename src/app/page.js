  ...
  return (
    <div className="flex flex-col gap-16">
      ...
      <section className="relative w-screen left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] overflow-hidden">
        <div className="relative flex flex-col items-center text-center px-6 md:px-10 pt-12 pb-12 md:pt-32 md:pb-32">
          ...
          <div className="relative max-w-4xl mx-auto">
            ...
            <h1 className="mt-6 md:mt-7 text-4xl md:text-6xl font-bold leading-tight tracking-tight text-zinc-900">
            ...
            <p className="mt-5 md:mt-6 text-base md:text-lg leading-relaxed text-zinc-600 max-w-2xl mx-auto">
              Scan a sidewall → instantly know the size & spec → contact nearby shops with confidence.
            </p>
            <ul className="mt-6 mb-6 md:mt-8 flex flex-wrap justify-center gap-2 text-[12px] font-medium">
              {['Decode size codes fast','Be prepared before you call','Find trusted local shops','From slow wear to sudden flats'].map(b=> (
                <li key={b} className="inline-flex items-center gap-1 rounded-full bg-white px-4 py-1 ring-1 ring-zinc-300 shadow-sm text-zinc-700 hover:ring-emerald-400/50 hover:text-zinc-900 transition"> 
      ...
      <section className="flex flex-col gap-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-2xl mx-auto">
          <div className="rounded-xl border border-amber-200/70 bg-amber-50/70 p-4 flex flex-col gap-3 shadow-sm">
            ...
            {showScanner && <div className="mt-5 block sm:hidden">{scannerPanel}</div>}
          </div>
          <div className="rounded-xl border border-violet-200/70 bg-violet-50/70 p-4 flex flex-col gap-3 shadow-sm">
            ...
            {showNearby && <div className="mt-5 block sm:hidden">{nearbyPanel}</div>}
          </div>
        </div>
        {/* Desktop & larger: panels rendered after cards */}
        {showScanner && <div className="hidden sm:block">{scannerPanel}</div>}
        {showNearby && <div className="hidden sm:block">{nearbyPanel}</div>}
      </section>
